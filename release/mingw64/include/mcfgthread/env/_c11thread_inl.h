...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN void call_once(once_flag *__once_c, void (*__func)(void)) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int cnd_init(cnd_t *__cond_c) _MCFCRT_NOEXCEPT {
	_MCFCRT_InitializeConditionVariable(__cond_c->__cond);
	return thrd_success;
}
__MCFCRT_C11THREAD_INLINE_OR_EXTERN void cnd_destroy(cnd_t *__cond_c) _MCFCRT_NOEXCEPT {
	(void)__cond_c;
}
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int cnd_wait(cnd_t *restrict __cond_c, mtx_t *restrict __mutex_c) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int cnd_signal(cnd_t *__cond_c) _MCFCRT_NOEXCEPT {
	_MCFCRT_SignalConditionVariable(__cond_c->__cond, 1);
	return thrd_success;
}
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int cnd_broadcast(cnd_t *__cond_c) _MCFCRT_NOEXCEPT {
	_MCFCRT_BroadcastConditionVariable(__cond_c->__cond);
	return thrd_success;
}

// 7.26.4 Mutex functions
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int mtx_init(mtx_t *__mutex_c, int __mask) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN void mtx_destroy(mtx_t *__mutex_c) _MCFCRT_NOEXCEPT {
	(void)__mutex_c;
}

__MCFCRT_C11THREAD_INLINE_OR_EXTERN int mtx_lock(mtx_t *__mutex_c) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int mtx_timedlock(mtx_t *restrict __mutex_c, const struct timespec *restrict __timeout) _MCFCRT_NOEXCEPT  {
	if(__mutex_c->__mask & mtx_recursive){
		const _MCFCRT_STD uintptr_t __self = _MCFCRT_GetCurrentThreadId();
		const _MCFCRT_STD uintptr_t __old_owner = __atomic_load_n(&(__mutex_c->__owner), __ATOMIC_RELAXED);
		if(_MCFCRT_EXPECT_NOT(__old_owner != __self)){
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int mtx_trylock(mtx_t *__mutex_c) _MCFCRT_NOEXCEPT {
	if(__mutex_c->__mask & mtx_recursive){
		const _MCFCRT_STD uintptr_t __self = _MCFCRT_GetCurrentThreadId();
		const _MCFCRT_STD uintptr_t __old_owner = __atomic_load_n(&(__mutex_c->__owner), __ATOMIC_RELAXED);
		if(_MCFCRT_EXPECT_NOT(__old_owner != __self)){
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int mtx_unlock(mtx_t *__mutex_c) _MCFCRT_NOEXCEPT {
	if(__mutex_c->__mask & mtx_recursive){
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int thrd_join(thrd_t __tid, int *__exit_code_ret) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int thrd_detach(thrd_t __tid) _MCFCRT_NOEXCEPT {
	if(__tid == _MCFCRT_GetCurrentThreadId()){
		return thrd_error; // XXX: EDEADLK
	}
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN thrd_t thrd_current(void) _MCFCRT_NOEXCEPT {
	return _MCFCRT_GetCurrentThreadId();
}
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int thrd_equal(thrd_t __tid1, thrd_t __tid2) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int tss_create(tss_t *__key_ret, tss_dtor_t __destructor) _MCFCRT_NOEXCEPT {
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN void tss_delete(tss_t __key) _MCFCRT_NOEXCEPT {
	_MCFCRT_TlsFreeKey(__key);
}
__MCFCRT_C11THREAD_INLINE_OR_EXTERN void *tss_get(tss_t __key) _MCFCRT_NOEXCEPT {
	void *__storage;
...
__MCFCRT_C11THREAD_INLINE_OR_EXTERN int tss_set(tss_t __key, void *__value) _MCFCRT_NOEXCEPT {
	void *__storage;
